...
private:
  ...
  const bool useExact, useCompressed, filter;
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr a;
...
public:
  Receiver(const std::string &topicColor, const std::string &topicDepth, const bool useExact, const bool useCompressed, const bool filter)
    : topicColor(topicColor), topicDepth(topicDepth), useExact(useExact), useCompressed(useCompressed), filter(filter),
      updateImage(false), updateCloud(false), save(false), save_seq(false), save_stop(false), running(false), frame(0), queueSize(5),
...
private:
  ...
  void start(const Mode mode)
  {
    ...
    a = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
  ...
  void cloudViewer(const bool filter)
  {
    cv::Mat color, depth;
    ...
    createCloud(depth, color, cloud, filter);
    ...
    visualizer->setCameraPosition(0, 0, 0, 0, 0, 0);
    ...
    for(; running && ros::ok();)
    {
      if(updateCloud)
      {
	      c = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
        ...
        createCloud(depth, color, cloud, filter);
          ...
          use_filter(cloud, a);
          visualizer->updatePointCloud(a, cloudName);
        }
  ...
  void createCloud(const cv::Mat &depth, const cv::Mat &color, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud, const bool filter) const
  {
    ...
    for(int r = 0; r < depth.rows; ++r)
    {
      ...
      for(size_t c = 0; c < (size_t)depth.cols; ++c, ++itP, ++itD, ++itC, ++itX)
      {
        ...
        if (filter){
          if (depthValue <1.2 && depthValue >0.5 && -y*depthValue > -0.2){
          itP->z = depthValue;
  ...
  void saveCloudAndImages(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
  {
    oss.str("");
  ...
  void createLookup(size_t width, size_t height)
  {
    ...
    //printf("fx:%d, fy:%d, cx:%d, cy:%d", fx,fy,cx,cy);
    float *it;
...
int main(int argc, char **argv)
{
  ...
  Receiver receiver(topicColor, topicDepth, useExact, useCompressed, filter);
